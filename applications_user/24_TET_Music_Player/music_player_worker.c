...
#define NOTE_C4_QUARTERTONE (4.0f * 24.0f)
#define TWO_POW_TWELTH_ROOT 1.059463094359f
#define TWO_POW_24TH_ROOT 1.029302236643f
...
static int32_t music_player_worker_thread_callback(void* context) {
    ...
    while(instance->should_work) {
        if(NoteBlockArray_end_p(it)) {
            ...
            // this is where the note frequency is calculated:
            // 1. subtract C4 frequency from this semitone (both of these are just whole numbers)
            // This is done to find the absolute position of this semitone in the entire scale.
            float note_from_a4_24TET = (float)note_block->semitone - NOTE_C4_SEMITONE;
            // 2. The difference between notes is a geometric relationship. To get from B to C, 
            // you simply multiply B by the common ratio between semitones to get C. In 12-TET,
            // this is, naturally, the 12th root of 2. In 24-TET, it would be the 24th root of 2.
            // In this line, the common ratio is being raised to the relative distance from A4.
            // This produces the difference in frequency from C4 to the desired note. 
            float frequency = NOTE_C4 * powf(TWO_POW_24TH_ROOT, note_from_a4_24TET);
            float duration =
